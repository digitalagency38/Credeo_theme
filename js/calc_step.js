...
// calc start
    ...
    function pageMains() {
        ...
        function priceRanges() {
            ...
            // $("#cash").slider({
            //     range: "min",
            //     value: window.config2.priceRange.value,
            //     min: window.config2.priceRange.min,
            //     step: window.config2.priceRange.step,
            //     max: window.config2.priceRange.max,
            ...
            $("#cash-pts2").slider({
                range: "min",
                value: window.config2.priceRangePTS2.value,
                min: window.config2.priceRangePTS2.min,
                step: window.config2.priceRangePTS2.step,
                max: window.config2.priceRangePTS2.max,
                slide: function(event, ui) {
                    $("#amount_cash-pts2").val(ui.value);
                    $("#myamsumm-pts").val(ui.value);
                    totalPTS();
                }
            });

            $("#amount_cash-pts2").val(($("#cash-pts2").slider("value")));
            $("#myamsumm-pts").val(($("#cash-pts2").slider("value")));
            $("#amount_cash-pts2").val(number_format($("#amount_cash-pts2").val()));
            $("#myamsumm-pts").val(number_format($("#amount_cash-pts2").val()));
            ...
            function total() {
                ...
                if (cash < 1000000) {
                    currentMonthPercent = window.config2.percents2.per5;
                } else if (cash > 1000000 && cash < 3000000) {
                    currentMonthPercent = window.config2.percents2.per4;
                } else {
                    currentMonthPercent = window.config2.percents2.per3;
                }
            ...
            function totalPTS() {
                var totalPayMonth = $('#month-pts2'),
                    days = $("#amount_time-pts2").val(),
                    cash = $("#amount_cash-pts2").val(),
                    totalFinal = $("#total-final-pts2"),
                    daysPercents = $(".js-days-percents-pts2"),
                    currentMonthPercent;

                if (cash <= 200000) {
                    currentMonthPercent = window.config2.percentsPTS2.per3;
                } else if (cash <= 450000 && cash >= 250000) {
                    currentMonthPercent = window.config2.percentsPTS2.per2;
                } else {
                    currentMonthPercent = window.config2.percentsPTS2.per1;
                }
            ...
            // $("#time").slider({
            //     range: "min",
            //     value: window.config2.daysRange.value,
            //     min: window.config2.daysRange.min,
            //     step: window.config2.daysRange.step,
            //     max: window.config2.daysRange.max,
            //     slide: function(event, ui) {
            ...
            if (window.config2.priceRange2.value < 1000000) {
                currentMonthPercent = window.config2.percents2.per5;
            } else if (window.config2.priceRange2.value > 1000000 && window.config2.priceRange2.value < 3000000) {
                currentMonthPercent = window.config2.percents2.per4;
            } else {
                currentMonthPercent = window.config2.percents2.per3;
            }
            ...
            $(".js-q-price-range-value").on("change", function() {
                if ($(this).val() > window.config2.priceRange2.max) {
                    $(this).val(window.config2.priceRange2.max)
                } else if ($(this).val() < window.config2.priceRange2.min) {
                    $(this).val(window.config2.priceRange2.min)
                }
            ...
            $(".js-q-days-range-value").on("change", function() {
                if ($(this).val() > window.config2.daysRange.max) {
                    $(this).val(window.config2.daysRange.max)
                } else if ($(this).val() < window.config2.daysRange.min) {
                    $(this).val(window.config2.daysRange.min)
                }
            ...
            $("#time-pts2").slider({
                range: "min",
                value: window.config2.daysRangePTS2.value,
                min: window.config2.daysRangePTS2.min,
                step: window.config2.daysRangePTS2.step,
                max: window.config2.daysRangePTS2.max,
                slide: function(event, ui) {
                    $("#amount_time-pts2").val(ui.value);
                    $("#myamtime-pts").val(ui.value);
                    $('.b-calc-param__textday-pts').html(getNumEnding($("#amount_time-pts2").val(), ['месяц', 'месяца', 'месяцев']));
                    $("#amount_cash-pts2").val(number_norm($("#amount_cash-pts2").val()));
                    $("#myamsumm-pts").val(number_norm($("#amount_cash-pts2").val()));
                    totalPTS();
                }
            });

            $("#amount_time-pts2").val($("#time-pts2").slider("value"));
            $("#myamtime-pts").val($("#time-pts2").slider("value"));


            var currentDayPTS = $("#amount_time-pts2").val(),
            currentPercentPTS,
            currentMonthPercentPTS;

            // if (window.config2.priceRangePTS2.value < 1000000) {
            //     currentMonthPercentPTS = window.config2.percentsPTS2.per3;
            // } else if (window.config2.priceRangePTS2.value > 1000000 && window.config2.priceRangePTS2.value < 3000000) {
            //     currentMonthPercentPTS = window.config2.percentsPTS2.per2;
            // } else {
            //     currentMonthPercentPTS = window.config2.percentsPTS2.per1;
            // }

            if (window.config2.priceRangePTS2.value <= 200000) {
                currentMonthPercentPTS = window.config2.percentsPTS2.per3;
            } else if (window.config2.priceRangePTS2.value <= 450000 && window.config2.priceRangePTS2.value >= 250000) {
                currentMonthPercentPTS = window.config2.percentsPTS2.per2;
            } else {
                currentMonthPercentPTS = window.config2.percentsPTS2.per1;
            }
            ...
            $("#month-pts2").html(($("#cash-pts2").slider("value") * currentPercentPTS).toFixed(0));
            $("#total-final-pts2").html(($("#cash-pts2").slider("value") + $("#cash-pts2").slider("value") * currentPercentPTS).toFixed(0));
            $("#month-pts2").html(number_format($("#month-pts2").html()));
            $("#total-pts").html(number_format($("#amount_cash-pts2").val()));
            $("#total-final-pts2").html(number_format($("#total-final-pts2").html()));

            $(".js-days-percents-pts2").html((currentMonthPercentPTS * 100).toFixed(1).replace(/\.0+$/, ""));

            $(".js-q-price-range-value-pts2").on("blur", function() {
                $("#amount_cash-pts2").val(number_format($("#amount_cash-pts2").val()));
            })

            $(".js-q-price-range-value-pts2, .js-q-days-range-value-pts2").on("focus", function() {
                $(this).val($(this).val().replace(/\s+/g, ''));
            })

            $(".js-q-price-range-value-pts2").on("change", function() {
                if ($(this).val() > window.config2.priceRangePTS2.max) {
                    $(this).val(window.config2.priceRangePTS2.max)
                } else if ($(this).val() < window.config2.priceRangePTS2.min) {
                    $(this).val(window.config2.priceRangePTS2.min)
                }
                $("#cash-pts2").slider("value", $(this).val());
                totalPTS();

            })
            $(".js-q-days-range-value-pts2").on("change", function() {
                if ($(this).val() > window.config2.daysRangePTS2.max) {
                    $(this).val(window.config2.daysRangePTS2.max)
                } else if ($(this).val() < window.config2.daysRangePTS2.min) {
                    $(this).val(window.config2.daysRangePTS2.min)
                }
                $("#time-pts2").slider("value", $(this).val());
                // var date = new Date();
                // date.setDate(date.getDate() + $("#time").slider("value"));
                // $(".js-last-payment-day").html(date.toLocaleString("ru", options));
                // $('.b-calc-param__textday').html(getNumEnding($("#amount_time").val(), ['день', 'дня', 'дней']));
            ...
            $(document).on("keydown", ".js-input-only-numbers2", function(e) {-1 !== $.inArray(e.keyCode, [46, 8, 9, 27, 13]) || (/65|67|86|88/.test(e.keyCode) && (e.ctrlKey === true || e.metaKey === true)) && (!0 === e.ctrlKey || !0 === e.metaKey) || 35 <= e.keyCode && 40 >= e.keyCode || (e.shiftKey || 48 > e.keyCode || 57 < e.keyCode) && (96 > e.keyCode || 105 < e.keyCode) && e.preventDefault() });
